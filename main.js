...
if (!trafficCtxNullable)
    throw new Error("TRAFFIC CANVAS NOT EXIST!");
if (!brainCtxNullable)
    throw new Error("BRAIN CANVAS NOT EXIST!");
//** CANVAS SETTINGS **//
...
if (!itemString) {
    ...
    const brainJson = JSON.parse(itemString);
    const brain = Object.setPrototypeOf(brainJson, Brain.prototype);
...
function animateTraffic() {
    ...
    aiCars.forEach(car => {
        if (car.pos.y - aiCars[0].pos.y > trafficCanvas.height)
            car.die = true;
        if (car.pos.x > trafficCanvas.width || car.pos.x < 0)
            car.die = true;
    });
    if (aiCars[0].die) {
