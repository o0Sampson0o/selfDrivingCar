...
function getIntersectionPoint(p1: Vector, p2: Vector, q1: Vector, q2: Vector) {
    const a1 = p2.y - p1.y;
    const b1 = p1.x - p2.x;
    ...
    if (p1_p2.dot(p1_point) < 0 || p1_point.length > p1_p2.length || q1_q2.dot(q1_point) < 0 || q1_point.length > q1_q2.length) return null;
    return point;
}
